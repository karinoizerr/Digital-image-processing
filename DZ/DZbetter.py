# -*- coding: utf-8 -*-
"""
Created on Sat Jun 13 18:59:11 2020
...
p0 = cv2.goodFeaturesToTrack(old_gray, mask = None, **feature_params) # Функция нахождения N самых сильных углов на изображении методом Ши-Томази. Передаётся кадр old_gray, параметры из feature_params.
                                                                      # Возвращает массив из пар-координатов углов (2D точек).
# Вывод первого кадра
...
count_2 = 0                                                     # Счётчик последующих кадров, в которых возможна регенерация точек.
cadr = 10                                                       # Количество начальных кадров, по которым определяется область объекта.
   ...
   # print("Оптический поток изначально вычилсен для: ", len(p1)," точек")
    if count <= cadr:                                           # Обработка первых кадров
        ...
        for i,new in enumerate(move):
            a,b = new.ravel()
            frame = cv2.circle(frame,(a,b),8,color[i].tolist(),-1)      # Рисование точки

    if count > cadr:                                                    # Обработка последующих кадров
        count_2 += 1                                                    # Счёт кадра с возможной регенрацией
        good_new = p1[st==1]                                            # Дальшнейшая обработка точек, для которых найден оптический поток
        new_center = np.median(good_new, axis=0)                        # Рассчёт нового центра прямоугольной области объекта, как медиана точек

        # Расчёт координат нижнего и верхнего углов прямоугольной области с объектом
        new_up = (new_center + (new_x, new_y)).reshape(-1)
        new_down = (new_center - (new_x, new_y)).reshape(-1)
        # Расчёт числа характерных точек в прямоугольной области с объектом
        in_box = 0
        for (x, y) in good_new:
            xy = np.array((x, y))
            if (xy >= new_down).all() and (xy <= new_up).all():
                in_box += 1
        #print("В области ", in_box," точек")
        # Регенерация новых характерные точек, если количество имеющихся в области объекта уменьшилось
        if in_box < number:
            # Изменение размерности массива для упрощения вычислений
            old_p0 = p0.reshape(p0.shape[0], p0.shape[2])
            # Регенерация новых характерных точек
            p0 = cv2.goodFeaturesToTrack(frame_gray, mask = None, **feature_params)
            #print("Регенерировано до ", p0.shape[0], " точек")
            if p0.shape[0] > number:
                # Формирование массива расстояний до центра прямоугольной области с объектом
                distances = []
                for xy in p0:
                    distances.append(np.linalg.norm(xy - new_center))
                # Выбор number точек, ближайших к центру прямоугольной области
                p0 = p0[np.argpartition(distances, number)[:number]]
            else:
                p0 = p0
            # Расчёт оптического потока с новыми характерными точками
            p1, st, err = cv2.calcOpticalFlowPyrLK(old_gray, frame_gray, p0, None, **lk_params)
            good_new = p1[st==1]
            if count_2 > 1:
                L = p1 - p0                                             # Расстояние, на которое перемещается точка с предыдущего кадра.
                good_new = []                                           # Создание пустого списка движущихся точек.
                for i in range(len(L)):                                 # Цикл для заполнения списка точками, переместившимися по сравнению с предыдущем кадром.
                    if (abs(L[i][0][0]) >= 0.1) or (abs(L[i][0][1] >= 0.1)): # Если точка переметсилась на пиксель по любому направлению.
                        good_new.append(p1[i][0])                       # Присоединеие точки к списку.
                good_new = np.array(good_new)
            #print("В итоге на объекте ", good_new.shape[0], " точек")
